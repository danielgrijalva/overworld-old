...
describe("Testing <EditProfile />", () => {
  ...
  const editProfileSpy = jest
    .spyOn(actions, "editProfile")
    .mockReturnValue(jest.fn());
  const refreshAvatarSpy = jest
    .spyOn(actions, "refreshAvatar")
    .mockReturnValue(jest.fn());
  ...
  const fillForm = profile => {
    wrap
      ...
      .simulate("change", {
        target: { name: "username", value: profile.username }
      });
    wrap
    ...
    wrap
      ...
      .simulate("change", {
        target: { name: "location", value: profile.location }
      });
    wrap
      .find('input[name="twitter"]')
      .simulate("change", {
        target: { name: "twitter", value: profile.twitter }
      });
    wrap
  ...
  beforeEach(() => {
    const initialStore = {
      auth: {
        token:
          "c95441469e6d9d3e6c699b44ee5b730aa4636d657f4917cf659319ad5c7c0dd6",
        ...
        user: {
          ...
          gravatar:
            "https://secure.gravatar.com/avatar/a8af925361e2a6a5869a3baf5a6e0ce1?size=120&default=retro"
        },
        errors: []
      }
    };
