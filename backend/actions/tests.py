...
class ActionsTests(APITestCase):
    def setUp(self):
        ...
        self.game = Game.objects.create(igdb=1074, name='Super Mario 64')
        self.client.force_authenticate(user=self.user)

    def test_get_actions(self):
        """Ensure we can retrieve the actions between a user and a game."""
        url = reverse('get-actions')
        data = {'igdb': self.game.igdb, 'name': self.game.name}
    ...
    def test_rate_game(self):
        ...
        url = reverse('rate-game')
        data = {'game': self.game.igdb, 'rating': 10}
    ...
    def test_invalid_rating_value(self):
        ...
        url = reverse('rate-game')
        negative_rating = {'game': self.game.igdb, 'rating': -1}
        big_rating = {'game': self.game.igdb, 'rating': 11}
